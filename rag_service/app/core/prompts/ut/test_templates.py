...
def load_template_files(templates_dir: Path) -> Dict[str, Dict[str, Any]]:
    ...
    for yaml_file in templates_dir.rglob("*.yaml"):
        ...
        templates[category].append(
            {"path": yaml_file, "name": yaml_file.stem, "category": category}
        )
    return templates
...
@pytest.mark.parametrize(
    "template_info",
    load_template_files(Path(__file__).parent.parent / "templates")["base"],
)
...
@pytest.mark.parametrize(
    "template_info",
    load_template_files(Path(__file__).parent.parent / "templates")["roles"],
)
def test_role_templates(templates_dir, template_info):
    ...
    result = prompt.render(
        domain="技术", expertise=["编程", "系统设计"], task_instruction="请回答以下问题"
    )
    assert "技术" in result
    assert "编程" in result
...
@pytest.mark.parametrize(
    "template_info",
    load_template_files(Path(__file__).parent.parent / "templates")["domains"],
)
...
@pytest.mark.parametrize(
    "template_info",
    load_template_files(Path(__file__).parent.parent / "templates")["tasks"],
)
