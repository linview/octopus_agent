...
class AgentExecutionResult(BaseModel):
    ...
    output_data: Any | None = Field(None, description="输出数据")
...
class WorkflowResult(BaseModel):
    ...
    start_time: datetime = Field(default_factory=datetime.now, description="开始时间")
    end_time: datetime | None = Field(None, description="结束时间")
    duration: float | None = Field(None, description="总执行时长（秒）")
    success: bool = Field(False, description="是否成功")
    error_message: str | None = Field(None, description="错误信息")
    ...
    style_profile: StyleProfile | None = Field(None, description="风格特征")
    generated_content: str | None = Field(None, description="生成的内容")
    ...
    def get_agent_result(self, agent_name: str) -> AgentExecutionResult | None:
        """获取指定Agent的执行结果"""
...
class ContentGenerationResult(WorkflowResult):
    ...
    style_profile_id: str | None = Field(None, description="风格特征ID")
    generation_quality_score: float | None = Field(None, description="生成质量评分")
    content_length: int | None = Field(None, description="内容长度")
    generation_time: float | None = Field(None, description="生成耗时")
...
class StyleLearningResult(WorkflowResult):
    ...
    feature_extraction_time: float | None = Field(None, description="特征提取耗时")
