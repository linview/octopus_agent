...
class StyleProfile(BaseModel):
    """写作风格特征模型"""
    profile_id: Optional[str] = Field(None, description="风格特征ID")
    ...
    sentence_structure: SentenceStructureFeatures = Field(default_factory=SentenceStructureFeatures, description="句式结构")
    ...
    def merge_with(self, other: "StyleProfile") -> "StyleProfile":
        ...
        merged.vocabulary.technical_terms = list(set(self.vocabulary.technical_terms + other.vocabulary.technical_terms))
        merged.vocabulary.vocabulary_richness = (self.vocabulary.vocabulary_richness + other.vocabulary.vocabulary_richness) / 2
        
        # 合并句式结构特征
        merged.sentence_structure.avg_sentence_length = (self.sentence_structure.avg_sentence_length + other.sentence_structure.avg_sentence_length) / 2
        merged.sentence_structure.transition_words = list(set(self.sentence_structure.transition_words + other.sentence_structure.transition_words))
        
        # 合并情感语调特征
        merged.emotion_tone.overall_sentiment = self.emotion_tone.overall_sentiment  # 保持原样
        merged.emotion_tone.sentiment_score = (self.emotion_tone.sentiment_score + other.emotion_tone.sentiment_score) / 2
        
        # 合并写作习惯特征
        merged.writing_habits.call_to_action = self.writing_habits.call_to_action or other.writing_habits.call_to_action
        merged.writing_habits.opening_patterns = list(set(self.writing_habits.opening_patterns + other.writing_habits.opening_patterns))
