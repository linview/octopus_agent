...
class BaseAgent(ABC):
    ...
    def _setup_logger(self):
        ...
        # 添加文件日志
        logger.add("logs/wx_agent.log", format=log_format, level="INFO", rotation="10 MB", retention="7 days")
    ...
    async def execute(self, input_data: Any) -> Any:
        ...
        try:
            ...
            self.logger.info(f"{self.agent_name} 执行成功，耗时: {execution_time:.2f}秒，" f"结果类型: {type(result)}")

            return result

        except Exception as e:
            execution_time = (datetime.now() - start_time).total_seconds()
            self.logger.error(f"{self.agent_name} 执行失败，耗时: {execution_time:.2f}秒，" f"错误: {str(e)}")
            raise
