...
class RoleBasedPrompt(BaseTemplate):
    ...
    def to_dict(self) -> Dict:
        data = super().to_dict()
        data.update({"role": self.role, "expertise": self.expertise})
        return data
...
class ChainOfThoughtPrompt(BaseTemplate):
    ...
    # 默认模板路径
    template_path: Path = (
        Path(__file__).parent / "templates" / "base" / "chain_of_thought.yaml"
    )
...
class FewShotPrompt(BaseTemplate):
    ...
    def to_dict(self) -> Dict:
        data = super().to_dict()
        data.update({"examples": self.examples})
        return data
...
class SelfConsistencyPrompt(BaseTemplate):
    ...
    # 默认模板路径
    template_path: Path = (
        Path(__file__).parent / "templates" / "base" / "self_consistency.yaml"
    )
    ...
    def to_dict(self) -> Dict:
        data = super().to_dict()
        data.update({"perspectives": self.perspectives})
        return data
...
class SelfCritiquePrompt(BaseTemplate):
    ...
    # 默认模板路径
    template_path: Path = (
        Path(__file__).parent / "templates" / "base" / "self_critique.yaml"
    )
    ...
    def to_dict(self) -> Dict:
        data = super().to_dict()
        data.update({"initial_answer": self.initial_answer})
        return data
...
class DomainPrompt(BaseTemplate):
    ...
    # 默认模板路径
    template_path: Path = (
        Path(__file__).parent / "templates" / "domains" / "technical.yaml"
    )
    ...
    def to_dict(self) -> Dict:
        data = super().to_dict()
        data.update({"domain": self.domain, "subdomains": self.subdomains})
        return data
...
class TaskPrompt(BaseTemplate):
    ...
    def to_dict(self) -> Dict:
        data = super().to_dict()
        data.update({"task_type": self.task_type, "steps": self.steps})
        return data
