...
class ArticleMeta(BaseModel):
    ...
    cover_image: str | None = Field(None, description="封面图片链接")
...
class ArticleFeatures(BaseModel):
    ...
    summary: str | None = Field(None, description="文章摘要")
    sentiment: str | None = Field(None, description="情感分析结果")
    reading_time: int | None = Field(None, description="预计阅读时间（分钟）")
    word_count: int | None = Field(None, description="字数统计")
    paragraph_count: int | None = Field(None, description="段落数量")
    has_images: bool = Field(False, description="是否包含图片")
    has_videos: bool = Field(False, description="是否包含视频")
    topic_category: str | None = Field(None, description="主题分类")
...
class Article(BaseModel):
    ...
    article_id: str | None = Field(None, description="文章唯一标识")
    title: str = Field(..., description="文章标题")
    ...
    local_path: str | None = Field(None, description="本地存储路径")
    ...
    def get_image_anchors(self) -> list[str]:
        """获取所有图片锚点"""
        return [f"$img_{img_id.replace('img_', '')}$" for img_id in self.images]
